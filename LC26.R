...
length(input[!duplicated(input)])

#hint duplicated


dupTest <- function(input){
  input0 = input
  for(i in 1:length(input)){
    if(i==1){
      input0[i] = F
    }else if(sum(input[1:i-1] == input[i])>=1){
      input0[i] = T
    }else{input0[i] = F}
  }
  return(input0)
...
lengthNoDup <- function(input){
  length(input[!dupTest(input)])
}

lengthNoDup(input)


lengthNoDup(c(1,4,6,2,7,2,1,8,9,6))